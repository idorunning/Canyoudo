...
// Geographic crime + outcomes for a set of city-centre points.
const crimeSnapshot = z.object({
  ...
  places: z.array(
    z.object({
      ...
      byCategory: z.array(countSchema).default([]),
      byOutcome: z.array(countSchema).default([]),
      series: z.array(z.object({ month: z.string(), total: z.number() })).default([]),
    })
  ),
});

// AI-drafted findings, generated by scripts/interpret-data.mjs from the snapshot
// above and published — clearly labelled — on /data/findings.
const findingsSnapshot = z.object({
  kind: z.literal('findings'),
  provenance: provenanceSchema,
  generatedAt: z.string(),
  model: z.string(), // human-readable model name, for the disclaimer
  headline: z.string(),
  summary: z.string(),
  items: z
...
const policeDataSchema = z.discriminatedUnion('kind', [nationalSnapshot, forceSnapshot, crimeSnapshot, findingsSnapshot]);
